...
void Application::Display(void)
{
	...
	case 3:
		m_pCamera->ResetCamera();
		//Move the camera on the x axis 30 units, look at the center of the scene, and make the -z axis the up direction of the camera
		m_pCamera->SetPositionTargetAndUp(vector3(30.0f, 0.0f, 0.0f), vector3(0.0f, 0.0f, 0.0f), vector3(0.0f, 0.0f, -1.0f));
		break;
	case 4:
		m_pCamera->ResetCamera();
		//Move the camera back on the z axis (look at the shapes from the other side)
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f));
		break;
	case 5:
		m_pCamera->ResetCamera();
		//Move the camera back on the z axis and set the near far planes so that the blue cone isn't visible
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f));
		m_pCamera->SetNearFar(vector2(5.0f, 15.0f));
		break;
	case 6:
		m_pCamera->ResetCamera();
		//Move the camera back on the z axis and set the near far planes so that the red torus isn't visible
		m_pCamera->SetPosition(vector3(0.0f, 0.0f, -15.0f));
		m_pCamera->SetNearFar(vector2(1.0f, 10.0f));
		break;
	case 7:
		m_pCamera->ResetCamera();
		//Change the up direction of the camera so the scene is upside-down
