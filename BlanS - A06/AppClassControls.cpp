...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::Add:
		//Add a level to the octree if the current level is less than 4
		if (m_uOctantLevels < 4)
		{
			//Clear the dimensions for all entities
			m_pEntityMngr->ClearDimensionSetAll();
			++m_uOctantLevels;
			
			//Delete the current octree and create a new one
			SafeDelete(octree);
			octree = new MyOctant(m_uOctantLevels, 5);
		}
		break;
	case sf::Keyboard::Subtract:
		//Subtract a level from the octree if the current level is greater than 0
