...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//Define an array to store the cube's corners
	vector3 cornerPoints[8];

	//Fill in the corner points based on the min and max points
	cornerPoints[0] = m_v3MinL;
	cornerPoints[1] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	cornerPoints[2] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	cornerPoints[4] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	cornerPoints[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	cornerPoints[5] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	cornerPoints[6] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	cornerPoints[7] = m_v3MaxL;

	//Loop through the corner points
	for (uint i = 0; i < 8; i++)
	{
		//Translate the corner point from local to global
		cornerPoints[i] = vector3(m_m4ToWorld * vector4(cornerPoints[i], 1.0f));

		//If this is the first point, set the min and max global points to the first point
		if (i == 0)
		{
			m_v3MinG = cornerPoints[i];
			m_v3MaxG = cornerPoints[i];
		}
		//For all other corner points
		else
		{
			//If the max's x component is less than the corner point's x component
			if (m_v3MaxG.x < cornerPoints[i].x)
			{
				//Update the max's x component
				m_v3MaxG.x = cornerPoints[i].x;
			}
			//If the min's x component is greater than the corner point's x component
			else if(m_v3MinG.x > cornerPoints[i].x)
			{
				//Update the min's x component
				m_v3MinG.x = cornerPoints[i].x;
			}
			//Do the same for the y component
			if (m_v3MaxG.y < cornerPoints[i].y)
			{
				m_v3MaxG.y = cornerPoints[i].y;
			}
			else if (m_v3MinG.y > cornerPoints[i].y)
			{
				m_v3MinG.y = cornerPoints[i].y;
			}
			//Do the same for the z component
			if (m_v3MaxG.z < cornerPoints[i].z)
			{
				m_v3MaxG.z = cornerPoints[i].z;
			}
			else if (m_v3MinG.z > cornerPoints[i].z)
			{
				m_v3MinG.z = cornerPoints[i].z;
			}
		}
	}
