...
class Application
{
	...
	std::vector<std::vector<vector3>> m_stopLists; //List of stop lists for each orbit
	std::vector<uint> m_currentStops; //Keeps track of each orbit's current stop
	float m_fLerpProgress = 0.0f; //The lerp progress for all orbits (min = 0.0f, max = less than 1.0f)
	String m_sProgrammer = "Stasha Blank - sob3966@rit.edu";
