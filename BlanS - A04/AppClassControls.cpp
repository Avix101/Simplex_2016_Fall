...
void Application::CameraRotation(float a_fSpeed)
{
	...
	//Rotate around the global y axis (for x mouse movement) and rotate around the camera's local right vector (for y mouse movement)
	//Y angle is negated to mimic the solution's camera controls (up mouse move for up camera rotate, etc.)
	m_pCamera->RotateCamera(AXIS_Y, -fAngleY);
	m_pCamera->RotateCamera(m_pCamera->GetRight(), fAngleX);
...
#pragma region Camera Position
	...
	//Keep track of forward / backwards and right / left movement
	float forwardMovement = 0.0f;
	float rightMovement = 0.0f;

	//Move forward fSpeed if W is being pressed
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
	{
		forwardMovement += fSpeed;
	}
	//Move backwards fSpeed if S is being pressed
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))
	{
		forwardMovement -= fSpeed;
	}
	//Move left fSpeed if A is being pressed
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A))
	{
		rightMovement -= fSpeed;
	}
	//Move right fSpeed if D is being pressed
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))
	{
		rightMovement += fSpeed;
	}

	//If the user has requested some type of camera movement
	if (abs(forwardMovement) > 0.0f || abs(rightMovement) > 0.0f)
	{
		//Move the camera
		m_pCamera->MoveCamera(forwardMovement, rightMovement);
	}
